...
export const Route = createFileRoute("/_authenticated/closet")({
  head: () => ({
    meta: [
      { title: "Closet · Virtual Lookbook" },
      { name: "description", content: "Your private digital closet — upload clothes you find online or shoot yourself, tag them, and send them to the AI styling studio." },
      { property: "og:title", content: "Closet · Virtual Lookbook" },
      { property: "og:description", content: "Your private digital closet of real garments, organized for AI try-on." },
      { property: "og:url", content: "https://virtuallookbookpro.lovable.app/closet" },
    ],
    links: [{ rel: "canonical", href: "https://virtuallookbookpro.lovable.app/closet" }],
  }),
    ...
    <div className={cn("group relative glass rounded-2xl overflow-hidden shadow-soft hover:shadow-glow transition-all", selected && "ring-2 ring-primary")}>
      ...
      <div className="p-3">
        ...
        <div className="mt-2 flex gap-1">
          ...
          <Button aria-label={`Edit ${item.name}`} size="sm" variant="outline" className="h-7 w-7 p-0" onClick={onEdit}><Pencil className="h-3 w-3" /></Button>
        </div>
      </div>
      {!selectMode && (
        <button onClick={onRemove} aria-label={`Delete ${item.name}`} className="absolute top-2 right-2 h-8 w-8 grid place-items-center rounded-full bg-background/80 opacity-0 group-hover:opacity-100 transition">
...
function ItemDialog({
  ...
  const runAutoFill = async (url: string, opts?: { sourceUrl?: string; canonical?: Record<string, any> }) => {
    ...
    try {
      ...
      setS((p) => ({
        ...
        notes: p.notes || (canonical.description ? String(canonical.description) : p.notes),
        source: opts?.sourceUrl || p.source,
      }));
      setAiSuggested(true);
    } catch (e: any) {
      console.error("Lovable AI autofill failed", e);
  ...
  const handleUrlImport = async (raw: string) => {
    ...
    try {
      ...
      // 3. Lovable AI fills only what URL metadata did not provide.
        ...
        <div className="px-6 pb-4 overflow-y-auto flex-1">
          ...
          {(analyzing || aiSuggested) && (
            <div className="glass rounded-xl px-3 py-2 mb-2 text-xs flex items-center gap-2">
              ...
              {analyzing ? "Lovable AI is reading the image…" : "Lovable AI suggested details ✦ — edit anything before saving"}
            </div>
          )}
