...
export const Route = createFileRoute("/_authenticated/moodboards")({
  head: () => ({
    meta: [
      { title: "Moodboards · Virtual Lookbook" },
      { name: "description", content: "Pin reference images and extract a color palette to set the mood for your next AI styling session." },
      { property: "og:title", content: "Moodboards · Virtual Lookbook" },
      { property: "og:description", content: "Visual moodboards with palette extraction to guide your styling." },
      { property: "og:url", content: "https://virtuallookbookpro.lovable.app/moodboards" },
    ],
    links: [{ rel: "canonical", href: "https://virtuallookbookpro.lovable.app/moodboards" }],
  }),
          ...
          <aside className="glass rounded-2xl p-2 max-h-[70vh] overflow-y-auto">
            {moodboards.map((m) => (
              <button key={m.id} onClick={() => setActiveId(m.id)}
                ...
                <button aria-label={`Delete moodboard ${m.name}`} onClick={(e) => { e.stopPropagation(); removeMoodboard(m.id); }}><Trash2 className="h-3 w-3 text-destructive" /></button>
              </button>
            ))}
          </aside>
      ...
      <div className="relative h-[70vh] rounded-3xl overflow-hidden bg-dreamy shadow-ring-soft">
        ...
        {board.pins.map((p) => (
          <div key={p.id} onPointerDown={drag(p.id)}
            ...
            <button aria-label="Remove pin" onClick={() => removePin(p.id)} className="absolute top-1 right-1 h-6 w-6 rounded-full bg-background/80 grid place-items-center opacity-0 group-hover:opacity-100"><Trash2 className="h-3 w-3" /></button>
          </div>
        ))}
        {board.pins.length === 0 && (
